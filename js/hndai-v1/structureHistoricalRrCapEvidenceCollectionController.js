(function (root, factory) {
    ...
    function dependencyError(dependencies) { for (var index = 0; index < DEPENDENCIES.length; index += 1) { var name = DEPENDENCIES[index][0], value = dependencies[name];
        ...
        if (typeof dependencies.pager.fetchClosedCandles !== "function" || typeof dependencies.replay.runReplay !== "function" ||
            ...
            typeof dependencies.scenario.analyzeScenarios !== "function" || typeof dependencies.collection.ingestWorkUnit !== "function" ||
            typeof dependencies.collection.createPilotManifest !== "function" || typeof dependencies.collection.validatePilotCheckpoint !== "function" ||
            typeof dependencies.collection.ingestPilotWorkUnit !== "function") return "DEPENDENCY_API_MISSING";
        return null; }
    ...
    function createController(options) {
        ...
        function pilotSnapshot(error) { return { schemaVersion: SCHEMA, mode: "PILOT_ONLY", source: SOURCE,
            countsTowardLiveReadiness: false, readiness: "NONE", state: state, error: error || null,
            checkpoint: clone(checkpoint), progress: checkpoint ? { completedUnits: checkpoint.cursor, totalUnits: 1,
                revision: checkpoint.revision, nextUnit: dependencies.collection.getPilotWorkUnit(checkpoint) } : null }; }
        ...
        async function executeUnit(unit, runToken, pilot) {
            var runtimeConfig = pilot ? dependencies.collection.getDefaultConfig() : checkpoint.config;
            ...
            var replayConfig = dependencies.replay.getDefaultConfig(); Object.assign(replayConfig, { symbol: unit.symbol, interval: unit.interval,
                warmupCandles: runtimeConfig.warmupCandles, maximumEvaluationCandles: Math.max(1, paged.candles.length - runtimeConfig.warmupCandles - runtimeConfig.horizonBars),
            ...
            var outcomePolicy = dependencies.outcome.getDefaultPolicy(); outcomePolicy.maximumForwardBars = runtimeConfig.horizonBars;
            ...
            var scenarioPolicy = dependencies.scenario.getDefaultPolicy(); scenarioPolicy.maximumForwardBars = runtimeConfig.horizonBars;
            ...
            if (runToken !== token || cancelRequested) return { cancelled: true };
            var unitEvidence = { unitId: unit.id, source: SOURCE, countsTowardLiveReadiness: false,
                gridValid: true, candleGrid: grid, inputChecksum: checksum(paged.candles), scenarioAnalysis: scenario };
            var ingested = pilot ? dependencies.collection.ingestPilotWorkUnit(checkpoint, unitEvidence) :
                dependencies.collection.ingestWorkUnit(checkpoint, unitEvidence);
        ...
        async function run() { if (running) return running; var error = dependencyError(dependencies); if (error || !checkpoint || !dependencies.collection.validateCheckpoint(checkpoint).valid) { state = "FAILED_CLOSED"; return snapshot(error || "INVALID_CHECKPOINT"); }
            ...
            running = (async function () { var completed = 0;
                while (completed < checkpoint.config.sessionUnitLimit) { var unit = dependencies.collection.getNextWorkUnit(checkpoint);
                    ...
                    var attempts = 0, result; do { result = await executeUnit(unit, runToken, false); if (result.valid || result.cancelled || !result.retryable) break; attempts += 1; } while (attempts <= checkpoint.config.retryLimit);
        ...
        async function createPilot(config) { var error = dependencyError(dependencies); if (error) { state = "PILOT_FAILED_CLOSED"; return pilotSnapshot(error); }
            checkpoint = dependencies.collection.createPilotManifest(config); if (!checkpoint) { state = "PILOT_FAILED_CLOSED"; return pilotSnapshot("INVALID_PILOT_CONFIG"); }
            await store.commit(checkpoint); state = "PILOT_READY"; return pilotSnapshot(); }
        async function restorePilot() { var error = dependencyError(dependencies); if (error) { state = "PILOT_FAILED_CLOSED"; return pilotSnapshot(error); }
            var loaded = await store.load(), validation = dependencies.collection.validatePilotCheckpoint(loaded);
            if (!validation.valid) { state = "PILOT_FAILED_CLOSED"; return pilotSnapshot(validation.error); }
            checkpoint = loaded; state = checkpoint.state === "PILOT_COMPLETED_PAUSED" ? "PILOT_COMPLETED_PAUSED" : "PILOT_PAUSED";
            return pilotSnapshot(); }
        async function startPilot() {
            if (running) return running; var error = dependencyError(dependencies);
            if (error || !checkpoint || !dependencies.collection.validatePilotCheckpoint(checkpoint).valid) {
                state = "PILOT_FAILED_CLOSED"; return pilotSnapshot(error || "INVALID_PILOT_CHECKPOINT");
            }
            if (checkpoint.cursor === 1 || checkpoint.state === "PILOT_COMPLETED_PAUSED") {
                state = "PILOT_COMPLETED_PAUSED"; return pilotSnapshot("PILOT_ALREADY_COMPLETED");
            }
            pauseRequested = false; cancelRequested = false; var runToken = ++token; state = "PILOT_RUNNING";
            running = (async function () { var unit = dependencies.collection.getPilotWorkUnit(checkpoint);
                if (!unit) { state = "PILOT_FAILED_CLOSED"; return pilotSnapshot("PILOT_UNIT_UNAVAILABLE"); }
                var runtimeConfig = dependencies.collection.getDefaultConfig(), attempts = 0, result;
                do { result = await executeUnit(unit, runToken, true); if (result.valid || result.cancelled || !result.retryable) break; attempts += 1; }
                while (attempts <= runtimeConfig.retryLimit);
                if (result.cancelled) { state = "PILOT_PAUSED"; await store.clearRunning(); return pilotSnapshot("PILOT_UNIT_CANCELLED"); }
                if (!result.valid) { state = result.retryable ? "PILOT_PAUSED_RETRYABLE" : "PILOT_FAILED_CLOSED"; await store.clearRunning(); return pilotSnapshot(result.error); }
                state = "PILOT_COMPLETED_PAUSED"; return pilotSnapshot();
            }()).finally(function () { running = null; }); return running;
        }
        function pausePilot() { pauseRequested = true; if (state === "PILOT_RUNNING") state = "PILOT_PAUSING"; return pilotSnapshot(); }
        function cancelPilot() { cancelRequested = true; token += 1; state = "PILOT_PAUSED"; return pilotSnapshot("PILOT_UNIT_CANCEL_REQUESTED"); }
        return { create: create, restore: restore, start: run, pause: pause, resume: resume, cancelCurrentUnit: cancel,
            createPilot: createPilot, restorePilot: restorePilot, startPilot: startPilot, resumePilot: startPilot,
            pausePilot: pausePilot, cancelPilotUnit: cancelPilot,
            getSnapshot: function () { return snapshot(); }, getPilotSnapshot: function () { return pilotSnapshot(); } };
    }
