...
test("dependency called", () => { let calls = 0; const x = browser({ evaluateHistoricalShadow() { calls++; return { comparison: "MATCH_BLOCK" }; } }); x.runReplay(candles(252), config()); assert.strictEqual(calls, 2); });
...
test("look-ahead protected", () => { const seen = []; const x = browser({ evaluateHistoricalShadow(prefix) { seen.push(prefix.length); return { comparison: "MATCH_ALLOW" }; } }); x.runReplay(candles(254), config()); assert.deepStrictEqual(seen, [251, 252, 253, 254]); });
...
test("deterministic keys", () => { const r = fixtureApi().runReplay(candles(251), config()); assert.strictEqual(r.observations[0].key, `BTCUSDT|15m|${candles(251)[250].closeTime}`); });
...
test("script order", () => assert.ok(html.indexOf("structureHistoricalLegacyCandidateAdapter.js") < html.indexOf("structureHistoricalShadowReplay.js") && html.indexOf("structureHistoricalReplayBinancePager.js") < html.indexOf("structureHistoricalShadowReplay.js") && html.indexOf("structureHistoricalShadowReplay.js") < html.indexOf("js/ui.js")));
