...
test("builder counters default safely", () => { const r = fixtureApi().runReplay(candles(256), config()); assert.deepStrictEqual([r.builderReadyCount,r.builderUnavailableCount,r.byBuilderStatus],[0,0,[]]); });
test("builder status sorting deterministic", () => { const x=browser({evaluateHistoricalShadow(){return{comparison:"NOT_COMPARABLE",builderStatusCounts:{ZONE_UNAVAILABLE:2,SIGNAL_UNAVAILABLE:2,INPUT_READY:1}};}}),r=x.runReplay(candles(251),config());assert.deepStrictEqual(r.byBuilderStatus,[{key:"SIGNAL_UNAVAILABLE",count:502},{key:"ZONE_UNAVAILABLE",count:502},{key:"INPUT_READY",count:251}]); });
...
test("direct decision evidence projected and exported", () => { const x=browser({evaluateHistoricalShadow(){return{comparison:"LEGACY_ALLOW_GATE_BLOCK",legacyDecision:"ALLOW",gateDecision:"BLOCK",legacyReason:"SETUP_CREATED",gateReason:"NO_ALIGNED_EVENT",legacyDecisionEvidence:{accepted:1},gateDecisionEvidence:{reason:"NO_ALIGNED_EVENT"}};}}),r=x.runReplay(candles(251),config()),parsed=JSON.parse(x.exportReplay(r));assert.deepStrictEqual([r.observations[0].legacyReason,r.observations[0].gateReason,parsed.observations[0].gateDecisionEvidence.reason],["SETUP_CREATED","NO_ALIGNED_EVENT","NO_ALIGNED_EVENT"]); });
